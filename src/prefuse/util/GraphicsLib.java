...
public class GraphicsLib {
     ...
     * {@link #COINCIDENT}, or {@link #PARALLEL}.
     */
    public static Object[] intersectLineLine(double a1x, double a1y, double a2x,
        double a2y, double b1x, double b1y, double b2x, double b2y, 
        Point2D intersectIn)
    {
    	Point2D intersect = intersectIn;
        ...
        if ( u_b != 0 ) {
            ...
            if ( 0 <= ua && ua <= 1 && 0 <= ub && ub <= 1 ) {
                ...
                return new Object[] {1, intersect};
            } else {
                return new Object[] {NO_INTERSECTION, intersect};
            }
        } else {
            return new Object[] {( ua_t == 0 || ub_t == 0 ? COINCIDENT : PARALLEL ), intersect};
        }
    }
     ...
     * {@link #COINCIDENT}, or {@link #PARALLEL}.
     */
    public static Object[] intersectLineRectangle(Point2D a1, Point2D a2, Rectangle2D r, Point2D[] pts) {
        ...
        if ( pts[0] == null ) pts[0] = new Point2D(0, 0);
        if ( pts[1] == null ) pts[1] = new Point2D(0, 0);
        
        int i = 0;
        if ( (int)intersectLineLine(mnx,mny,mxx,mny,a1x,a1y,a2x,a2y,pts[i])[0] > 0 ) {
        	pts[i] = (Point2D)intersectLineLine(mnx,mny,mxx,mny,a1x,a1y,a2x,a2y,pts[i])[1];
        	i++;
        }
        if ( (int)intersectLineLine(mxx,mny,mxx,mxy,a1x,a1y,a2x,a2y,pts[i])[0] > 0 ) {
        	pts[i] = (Point2D)intersectLineLine(mxx,mny,mxx,mxy,a1x,a1y,a2x,a2y,pts[i])[1];
        	i++;
        }
        if ( i == 2 ) return new Object[] {i, pts};
        if ( (int)intersectLineLine(mxx,mxy,mnx,mxy,a1x,a1y,a2x,a2y,pts[i])[0] > 0 ) {
        	pts[i] = (Point2D)intersectLineLine(mxx,mxy,mnx,mxy,a1x,a1y,a2x,a2y,pts[i])[1];
        	i++;
        }
        if ( i == 2 ) return new Object[] {i, pts};
        if ( (int)intersectLineLine(mnx,mxy,mnx,mny,a1x,a1y,a2x,a2y,pts[i])[0] > 0 ) {
        	pts[i] = (Point2D)intersectLineLine(mnx,mxy,mnx,mny,a1x,a1y,a2x,a2y,pts[i])[1];
        	i++;
        }
        return new Object[] {i, pts} ;
    }
    ...
    public static void expand(Rectangle2D r, double amount) {
        r = new Rectangle2D(r.getMinX()-amount, r.getMinY()-amount,
