...
public class FontLib {
    ...
    public static Font getFont(String name, double size) {
        ...
        return getFont(name, "Normal", isize);
    }
     ...
     * @return the requested Font instance
     */
    public static Font getFont(String name, String style, double size) {
        int isize = (int)Math.floor(size);
    ...
    public static Font getFont(String name, String style, int size) {
        int key = (name.hashCode()<<8)+(size<<2)+style.hashCode();
        Font f = null;
        if ( (f=(Font)fontMap.get(key)) == null ) {
            f = Font.font(name, FontWeight.findByName(style), size);
            fontMap.put(key, f);
            misses++;
        }
        lookups++;
